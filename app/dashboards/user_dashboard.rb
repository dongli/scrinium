require "administrate/base_dashboard"

class UserDashboard < Administrate::BaseDashboard
  # ATTRIBUTE_TYPES
  # a hash that describes the type of each of the model's fields.
  #
  # Each different type represents an Administrate::Field object,
  # which determines how the attribute is displayed
  # on pages throughout the dashboard.
  ATTRIBUTE_TYPES = {
    resources: Field::HasMany,
    folders: Field::HasMany,
    shares: Field::HasMany,
    messages: Field::HasMany.with_options(class_name: "Mailboxer::Message"),
    receipts: Field::HasMany.with_options(class_name: "Mailboxer::Receipt"),
    memberships: Field::HasMany,
    organizations: Field::HasMany,
    groups: Field::HasMany,
    active_relationships: Field::HasMany.with_options(class_name: "Relationship"),
    following: Field::HasMany.with_options(class_name: "User"),
    passive_relationships: Field::HasMany.with_options(class_name: "Relationship"),
    followers: Field::HasMany.with_options(class_name: "User"),
    articles: Field::HasMany,
    comments: Field::HasMany,
    posts: Field::HasMany,
    publications: Field::HasMany,
    references: Field::HasMany,
    collections: Field::HasMany,
    profile: Field::HasOne,
    experience: Field::HasOne,
    achievement: Field::HasOne,
    user_option: Field::HasOne,
    user_quotum: Field::HasOne,
    id: Field::Number,
    name: Field::String,
    email: Field::String,
    mobile: Field::String,
    encrypted_password: Field::String,
    role: Field::String,
    deleted_at: Field::DateTime,
    reset_password_token: Field::String,
    reset_password_sent_at: Field::DateTime,
    remember_created_at: Field::DateTime,
    confirmation_token: Field::String,
    confirmed_at: Field::DateTime,
    confirmation_sent_at: Field::DateTime,
    unconfirmed_email: Field::String,
    authentication_token: Field::String,
    created_at: Field::DateTime,
    updated_at: Field::DateTime,
  }

  # COLLECTION_ATTRIBUTES
  # an array of attributes that will be displayed on the model's index page.
  #
  # By default, it's limited to four items to reduce clutter on index pages.
  # Feel free to add, remove, or rearrange items.
  COLLECTION_ATTRIBUTES = [
    :name,
    :email,
    :role
  ]

  # SHOW_PAGE_ATTRIBUTES
  # an array of attributes that will be displayed on the model's show page.
  SHOW_PAGE_ATTRIBUTES = ATTRIBUTE_TYPES.keys

  # FORM_ATTRIBUTES
  # an array of attributes that will be displayed
  # on the model's form (`new` and `edit`) pages.
  FORM_ATTRIBUTES = [
    :resources,
    :folders,
    :shares,
    :messages,
    :receipts,
    :memberships,
    :organizations,
    :groups,
    :active_relationships,
    :following,
    :passive_relationships,
    :followers,
    :articles,
    :comments,
    :posts,
    :publications,
    :references,
    :collections,
    :profile,
    :experience,
    :achievement,
    :user_option,
    :user_quotum,
    :name,
    :email,
    :mobile,
    :encrypted_password,
    :role,
    :deleted_at,
    :reset_password_token,
    :reset_password_sent_at,
    :remember_created_at,
    :confirmation_token,
    :confirmed_at,
    :confirmation_sent_at,
    :unconfirmed_email,
    :authentication_token,
  ]

  # Overwrite this method to customize how users are displayed
  # across all pages of the admin dashboard.
  #
  # def display_resource(user)
  #   "User ##{user.id}"
  # end
end
